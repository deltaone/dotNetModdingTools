...
namespace dotNetMT
{
    public static class IL
    {
        ...
        /// Returns a method definition.
        /// </summary>
        public static MethodDefinition GetMethodDefinition(ModuleDefinition moduleDefinition, string typeName, string methodName, int parameterCount = -1)
        {
            TypeDefinition t = GetTypeDefinition(moduleDefinition, typeName);
            if (t == null) 
                return (null);
            return (GetMethodDefinition(t, methodName, parameterCount));
        }
